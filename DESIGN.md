...
The Foo Academy is a fictitious learning platform that will allow members to upload and share online courses. The Academy would like an application to manage membership and course access.

Tracker is a member and course management system that will allow authenticated and authorized members to manage membership and course access
...
- The application must store member information, including but not limited to member name, email address, and password hash.
...
Roles of the members who will interact with the Tracker application:
...
| Assign Members to Course Form | Yes      | Allows course owners, editors, and teachers to assign other members to one of their courses.                                   |
...
| Add Member Form               | Yes      | Allows administrators to directly add members without a request for membership.                                                |
| Edit Member Form              | Yes      | Allows administrators to edit member profiles.                                                                                 |
| Delete Member Form            | Yes      | Allows administrators to remove member access to Tracker features and content.                                                 |
| Request Membership Form       | No       | Accepts requests for membership in the Foo Academy from anonymous users.                                                       |
| Approve Member Form           | Yes      | Allows administrators to approve requests for membership in the Foo Academy from anonymous users.                              |
...
| List of Members (Web UI)           | Yes                     | Displays a sortable and searchable list of members in the Foo Academy, with links to add, edit, and delete members.                                                                   |
| List of Members (API)              | Yes                     | Display a list of members and their details in JSON format.                                                                                                                           |
| View Member Profile (Web UI)       | Yes                     | Display the details of a member, like member name and email.                                                                                                                          |
| View Member Profile (API)          | Yes                     | Display the details of a member, like member name and email, in JSON format.                                                                                                          |
...
|     member                            |        |
+---------------------------------------+        |
| PK  member_id           INTEGER NN UQ |<--1:N--|
+---------------------------------------+        |
|     member_name         TEXT    NN UQ |        |
|     member_email        TEXT    NN UQ |        |
|     member_is_approved  INTEGER NN    |        |
|     member_is_admin     INTEGER NN    |        |
|     member_is_flagged   INTEGER NN    |        |
|     member_flagged_by   TEXT          |        |
