...
Usage:
- (Interactive) coverage run -m unittest --buffer --verbose tracker_XX/tests/test_app.py
- (Auto) echo 'default' | coverage run -m unittest --buffer --verbose tracker_XX/tests/test_app.py

> **NOTE** - coverage.py will generate a `__pycache__` folder.
...
from tracker.tracker_04a import create_app, check_system
